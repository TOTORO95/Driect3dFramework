...
//	{ MessageBoxW(NULL, _message, L"System Message",MB_OK); /*__ASM { int 3 };*/return _return;}
	...
	#define NULL_CHECK_RETURN( _ptr, _return)	\
	{if( _ptr == 0){/*__ASM { int 3 };*/  return S_OK;}}

	#define NULL_CHECK_MSG( _ptr, _message )		\
	{if( _ptr == 0){MessageBox(NULL, _message, L"System Message",MB_OK);/*__ASM { int 3 };*/}}

	#define NULL_CHECK_RETURN_MSG( _ptr, _return, _message )	\
	{if( _ptr == 0){MessageBox(NULL, _message, L"System Message",MB_OK);/*__ASM { int 3 };*/return S_OK;}}
	...
	#define FAILED_CHECK_RETURN(_hr, _return)	if( ((HRESULT)(_hr)) < 0 )		\
	{ MessageBoxW(NULL, L"Failed", L"System Error",MB_OK); /*__ASM { int 3 };*/ return S_OK;}
